...
personalidades = [
    ...
    "Impaciente e que quer pular etapas, sempre perguntando 'mas pra que isso serve na prática?'",
    "Extremamente ansioso, que interpreta qualquer erro como sinal de que não nasceu pra isso",
    "Que só aprende com exemplo, sempre pedindo pro professor um exemplo prático",
    "Que confunde conceitos com o tema atual, misturando tudo na mesma resposta",
    "Perfeccionista, que trava em detalhes pequenos e não consegue avançar sem entender tudo a fundo",
    "Competitivo, que compara tudo com como seria feito de outro jeito que ele aprendeu no YouTube",
    "Que questiona tudo filosoficamente antes de aceitar qualquer conceito, perguntando 'mas por que isso é assim?'",
    "Que depende totalmente do professor e não tenta resolver nada sozinho antes de pedir ajuda",
    "Que entende a teoria perfeitamente mas trava completamente quando precisa aplicar na prática",
    "Que mistura português com termos em inglês do jeito errado, criando confusão nos conceitos",
    "Que só aprende por repetição e precisa ver o mesmo exemplo várias vezes de formas diferentes",
    "Que tem muita criatividade mas vai sempre na direção errada, chegando em soluções mirabolantes",
    "Que entende tudo mas não acredita em si mesmo, precisando de validação constante pra continuar",
    "Que só se engaja quando o tema tem alguma conexão com jogos ou cultura pop, ignorando o resto",
    "Que resolve tudo certo mas nunca consegue explicar como chegou na resposta",
    "Que tem medo de parecer burro e prefere ficar em silêncio a arriscar uma resposta errada",
    "Que entende errado na primeira explicação e fica convicto do entendimento errado até o fim",
    "Que só consegue aprender se sentir que foi ele mesmo quem descobriu, não quem foi ensinado",
    "Que tem facilidade com a matéria mas é completamente desorganizado",
    "Que precisa contextualizar historicamente qualquer conceito antes de aceitar aprender sobre ele",
    "Que responde sempre com excesso de detalhes irrelevantes, tornando simples respostas em monólogos longos",]
...
def gerar_perfil():
    ...
    adicionar_balao_chat(None, f"📢 O professor entrou na sala.", "sistema")
    #
...
def iniciar_simulacao():
    ...
    prompt_sistema = (
        ...
        f"Você está em uma aula com seu professor sobre o tema: '{tema.get()}'. "
...
def enviar_mensagem_professor():
    ...
    if not texto_professor or not simulacao_ativa: return
    #
    adicionar_balao_chat("Você (Professor)", texto_professor, "professor")
...
def finalizar_aula():
    ...
    prompt_aval = [{
        "role": "user",
        "content": (
            f"Você é um pedagogo especialista em ensino. Avalie a seguinte sequência de respostas que um professor deu "
...
btn_entrar_aula.grid(row=1, column=0, padx=20, pady=(0, 15), sticky="ew")
#
tk.Frame(painel_esquerdo, bg="#29292e", height=1).grid(row=2, column=0, sticky="ew", padx=20, pady=(0, 20))
tk.Label(painel_esquerdo, text="Configuração da Aula", font=("Consolas", 16, "bold"), bg="#202024", fg="#e1e1e6").grid(row=3, column=0, sticky="w", padx=20, pady=(10, 20))
tk.Label(painel_esquerdo, text="Qual o tema/conceito da aula?", font=("Consolas", 11), bg="#202024", fg="#e1e1e6").grid(row=4, column=0, sticky="w", padx=20, pady=(5, 5))
#
tema = tk.Entry(painel_esquerdo, font=("Consolas", 12), bg="#121214", fg="black", insertbackground="black", bd=1, relief="solid")
tema.insert(0, " ")
tema.grid(row=5, column=0, padx=20, pady=(0, 20), sticky="ew", ipady=8)
tema.config(state="disabled", disabledbackground="#202024", disabledforeground="#8f8f98")
#
...
comecar.grid(row=6, column=0, padx=20, pady=(0, 15), sticky="ew")
comecar.config(state="disabled", bg="#444449", fg="#8f8f98")
#
...
btn_sair_aula.grid(row=7, column=0, padx=20, pady=(0, 15), sticky="ew")
btn_sair_aula.config(state="disabled", bg="#444449", fg="#8f8f98")
#
...
btn_historico.grid(row=8, column=0, padx=20, pady=(0, 15), sticky="ew")
btn_historico.config(state="disabled", bg="#444449", fg="#8f8f98")
#
...
btn_sobre.grid(row=9, column=0, padx=20, pady=(0, 15), sticky="ew")
btn_sobre.config(state="disabled", bg="#444449", fg="#8f8f98")
#
overlay = tk.Frame(painel_esquerdo, bg="#202024", width=300, height=620)
overlay.place(x=0, y=145, relwidth=1, relheight=0.75)
tk.Label(overlay, text="🔒\nDesbloqueia após entrar \nna sala de aula pela primeira vez", font=("Consolas", 11, "italic"), bg="#202024", fg="#555560", justify="center").place(relx=0.5, rely=0.5, anchor="center")
#
